...
# condition
# countries = ['us', 'de', 'fr', 'jp', 'es', 'gb']
countries = ['us', 'fr', 'jp', 'es', 'gb']
...
def get_youtube_info(sb_youtube_info_line):
	...
	with threading.Semaphore(10):
		...
		try:
			...
			# 每條Thread完成時間不同, 故印出不會依照順序 
			print(time.strftime('%X', time.localtime()), country, 'rank:', sb_rank, '-> %s is running...' % threading.current_thread().name)
...
for sb_youtube_info_line in sb_youtube_info:
	...
	task = threading.Thread(target=get_youtube_info, args=(sb_youtube_info_line,))  # args固定格式(x,y)
...
print(time.strftime('%X', time.localtime()),'Total', threading.active_count(), 'Thread Is Starting Crawlerd !!')

counter = 1
for task in tasks:
	counter += 1
	task.join()   # 每個線程加入join, 代表跑完才會執行下一步
